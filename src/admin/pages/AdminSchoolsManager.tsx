    ...
    openCropper(
      ...
      async (croppedFile) => {
        ...
        if (res.success && res.publicUrl) {
          ...
          else setIconUrl(res.publicUrl);
        } else {
          alert(`Upload failed: ${res.error}`);
        }
      },
      opts || { aspectRatio: 1, cropShape: 'round', title: 'Crop School Icon/Badge (1:1 Circle)' }
    );
  };
          ...
          <AdminFormGroup label="School Icon / Logo Upload">
            <div className="flex gap-2">
              ...
              <label className="px-3 py-2 bg-[#F3F4F6] hover:bg-[#E5E7EB] text-[#1F2937] text-xs font-semibold rounded-md cursor-pointer flex items-center gap-1.5 flex-shrink-0 border border-[#E5E7EB]">
                ...
                <input type="file" accept="image/*" className="hidden" onChange={(e) => handleFileUpload(e, (url) => setIconUrl(url), { aspectRatio: 1, cropShape: 'round', title: 'Crop School Badge Icon (1:1 Circle)' })} />
              </label>
            </div>
          </AdminFormGroup>
        </div>
