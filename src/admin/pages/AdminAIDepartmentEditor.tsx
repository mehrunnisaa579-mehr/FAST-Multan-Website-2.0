...
export default function AdminAIDepartmentEditor() {
  ...
  // Head / Incharge Message Section State
  const [hodHeading, setHodHeading] = useState("HOD'S MESSAGE");
  const [hodName, setHodName] = useState('Dr. Head of Department');
  const [hodDesignation, setHodDesignation] = useState('Head, Department of Artificial Intelligence & Data Science');
  ...
  const [viewAllProgramsUrl, setViewAllProgramsUrl] = useState('/departments/computing/ai-data-science/programs');
  const [programsList, setProgramsList] = useState<AIProgramItem[]>(
    aidsPrograms.map((p, idx) => ({
      id: p.id,
      title: p.title,
      subtitle: p.subtitle,
      imageLabel: p.imageLabel,
      image: '',
      url: '/departments/computing/ai-data-science/programs',
      display_order: idx + 1,
      is_visible: true,
    }))
  );
  ...
  const [viewAllFacultyUrl, setViewAllFacultyUrl] = useState('/departments/computing/ai-data-science/faculty');
  const [facultyList, setFacultyList] = useState<AIFacultyItem[]>(
    aidsFaculty.map((f, idx) => ({
      id: f.id,
      name: f.name,
  ...
  const [editingProg, setEditingProg] = useState<Partial<AIProgramItem> | null>(null);
  const [deleteProgTarget, setDeleteProgTarget] = useState<AIProgramItem | null>(null);

  const [isFacModalOpen, setIsFacModalOpen] = useState(false);
  const [editingFac, setEditingFac] = useState<Partial<AIFacultyItem> | null>(null);
  const [deleteFacTarget, setDeleteFacTarget] = useState<AIFacultyItem | null>(null);

  const [isAlliedModalOpen, setIsAlliedModalOpen] = useState(false);
  const [editingAllied, setEditingAllied] = useState<Partial<AIFacultyItem> | null>(null);
  const [deleteAlliedTarget, setDeleteAlliedTarget] = useState<AIFacultyItem | null>(null);

  const [saving, setSaving] = useState(false);
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const fetchData = async () => {
    const saved = await cmsService.getSetting<any>('department_ai_content', null);
    if (saved) {
  ...
  const handleSaveProg = () => {
    ...
    if (idx >= 0) {
      updated[idx] = editingProg as AIProgramItem;
    } else {
      updated.push(editingProg as AIProgramItem);
    }
  ...
  const handleOpenAddAllied = () => {
    setEditingAllied({
      id: `allied-ai-${Date.now()}`,
      name: 'Dr. New Allied Faculty',
      designation: 'Associated Professor (AI & DS)',
  ...
  const handleSaveAlliedModal = (savedData: FacultyMemberData) => {
    ...
    const itemToSave = { ...editingAllied, ...savedData } as AIFacultyItem;
    const idx = updated.findIndex((f) => f.id === itemToSave.id);
    if (idx >= 0) {
      updated[idx] = itemToSave;
    } else {
  ...
  const handleSaveAll = async () => {
    ...
    const res = await cmsService.saveSetting('department_ai_content', payload, 'Department of AI Content');

    if (res.success) {
      setMessage({ type: 'success', text: 'Department of AI page content saved successfully.' });
  ...
  return (
    <div className="space-y-6 text-left max-w-[1250px]">
      <div className="flex items-center gap-4 mb-2">
        ...
        <AdminPageHeader
          title="Department of AI Editor"
          subtitle="No-code control for public Department of AI page, Hero, Head/Incharge Message, Programs, and Faculty."
          action={
        ...
        {accordions.hero && (
          <div className="p-5 space-y-4">
            <AdminFormGroup label="Department Page Title">
              <AdminInput value={heroTitle} onChange={(e) => setHeroTitle(e.target.value)} placeholder="Department of AI" />
            </AdminFormGroup>
      ...
      <div className="border border-[#E5E7EB] rounded-lg bg-white overflow-hidden shadow-xs">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span>Head / Incharge Message</span>
          </div>
        ...
        {accordions.hod && (
          <div className="p-5 space-y-4">
            <AdminFormGroup label="Section Heading">
              <AdminInput value={hodHeading} onChange={(e) => setHodHeading(e.target.value)} placeholder="HOD'S MESSAGE or INCHARGE'S MESSAGE" />
            </AdminFormGroup>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <AdminFormGroup label="Head / Incharge Name">
                <AdminInput value={hodName} onChange={(e) => setHodName(e.target.value)} placeholder="Dr. Head/Incharge Name" />
              </AdminFormGroup>

              <AdminFormGroup label="Designation (Editable)">
                <AdminInput value={hodDesignation} onChange={(e) => setHodDesignation(e.target.value)} placeholder="Incharge, Department of AI" />
              </AdminFormGroup>
            </div>

            <AdminFormGroup label="Head / Incharge Photograph Upload">
              <div className="flex items-center gap-4">
            ...
            <AdminFormGroup label="Message / Overview Paragraph">
        ...
        {accordions.programs && (
          <div className="p-5 space-y-4">
            ...
            <div className="flex justify-between items-center pt-2">
              <h4 className="text-sm font-bold text-[#374151]">AI Degree Programs List</h4>
      ...
      <div className="border border-[#E5E7EB] rounded-lg bg-white overflow-hidden shadow-xs">
        ...
        {accordions.faculty && (
          <div className="p-5 space-y-4">
            ...
            <div className="flex justify-between items-center pt-2">
              <h4 className="text-sm font-bold text-[#374151]">AI Faculty List</h4>
      ...
      <AdminModal
        ...
        title={editingProg?.id ? 'Edit AI Program' : 'Add AI Program'}
        maxWidth="md"
        footer={
          <>
      ...
      <FacultyEditModal
        ...
        title={editingAllied?.id ? 'Edit AI Allied Faculty' : 'Add AI Allied Faculty'}
