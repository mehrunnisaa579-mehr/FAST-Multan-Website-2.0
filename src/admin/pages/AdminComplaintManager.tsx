...
export default function AdminComplaintManager() {
  ...
  const handleAddStep = () => {
    setSteps([
      ...
      {
        id: `step-${Date.now()}`,
        title: 'New Complaint Step',
        description: 'Instructions for submitting or resolving complaints...',
        iconUrl: '',
        visible: true,
      },
    ]);
  ...
  const handleSaveAll = async () => {
    ...
    const res = await cmsService.saveSetting('complaint_management_content', payload, 'Complaint Management System Content');
    setIsSaving(false);

    if (res.success) {
      setMessage({ type: 'success', text: 'Complaint Management System settings saved successfully.' });
    ...
    <div className="space-y-6 text-left max-w-[1300px]">
      <div className="flex items-center gap-4 mb-2">
        ...
        <AdminPageHeader
          ...
          subtitle="Manage the Complaint Management System page, hero media, instructions, online portal links, and contact information."
          action={
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-[#6B7280]">
              Control whether Complaint Management System is enabled and visible on the public website and navigation menus.
            </p>
          </div>
          <AdminToggle
      ...
      <AdminCard className="space-y-4">
        <h3 className="text-base font-bold text-[#1F2937] border-b border-[#F3F4F6] pb-2">
          2. Main Overview & Online Portal Access Link
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2 border-t border-[#F3F4F6]">
          <AdminFormGroup label="CTA Button Text">
            <AdminInput value={buttonText} onChange={(e) => setButtonText(e.target.value)} placeholder="Submit / Track Complaint Online" />
          </AdminFormGroup>

          <AdminFormGroup label="Portal Action Link / URL">
            <AdminInput value={buttonUrl} onChange={(e) => setButtonUrl(e.target.value)} placeholder="https://flexstudent.nu.edu.pk/" />
          </AdminFormGroup>
        </div>

        <AdminFormGroup label="Helpline & Support Info Box">
      ...
      <AdminCard className="space-y-6">
        <div className="flex items-center justify-between border-b border-[#F3F4F6] pb-3">
          <div>
            <h3 className="text-base font-bold text-[#1F2937]">3. Complaint Filing Steps & Instructions</h3>
            <p className="text-xs text-[#6B7280]">Add, edit, reorder, or show/hide steps on how to file a complaint.</p>
          </div>
        ...
        <div className="space-y-4">
          {steps.map((step, idx) => (
            <div key={step.id || idx} className="p-4 border border-[#E5E7EB] rounded-lg bg-white space-y-4">
              ...
              <AdminFormGroup label="Step Instructions / Details">
                <AdminTextarea
                  rows={2}
