...
function GalleryRow({ items, rowLabel }: { items: AboutGalleryItem[]; rowLabel: string }) {
  ...
  return (
    <div className="relative w-full mb-[28px] last:mb-0">
      <div
        ref={scrollRef}
        className="flex gap-[20px] overflow-x-auto scroll-smooth no-scrollbar w-full py-1"
        style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}
      >
        {items.map((item) => (
          <div
            key={item.id}
            className="w-[82vw] sm:w-[calc(50%-10px)] lg:w-[calc(33.333%-14px)] flex-shrink-0 bg-white border border-[#EAEAEA] rounded-[4px] overflow-hidden shadow-sm flex flex-col group cursor-pointer"
          >
            <div className="w-full aspect-[4/3] bg-[#D9D9D9] flex items-center justify-center p-[16px] relative overflow-hidden">
              {item.image ? (
                <img
                  src={item.image}
                  alt={item.title}
...
export default function AboutGallerySlider() {
  const [row1Items, setRow1Items] = useState<AboutGalleryItem[]>(aboutGalleryImages.slice(0, 6));
  const [row2Items, setRow2Items] = useState<AboutGalleryItem[]>(aboutGalleryImages.slice(6, 12));
  const [row3Items, setRow3Items] = useState<AboutGalleryItem[]>(aboutGalleryImages.slice(12, 18));
  ...
  useEffect(() => {
    const fetchGallery = async () => {
      ...
      if (cmsItems && cmsItems.length > 0) {
        ...
        const r1 = formatted.filter((i) => i.row_number === 1);
        const r2 = formatted.filter((i) => i.row_number === 2);
        const r3 = formatted.filter((i) => i.row_number === 3);

        if (r1.length > 0) setRow1Items(r1);
        if (r2.length > 0) setRow2Items(r2);
        if (r3.length > 0) setRow3Items(r3);
      }
    };
    fetchGallery();
  }, []);
  ...
  return (
    <section className="w-full my-[45px]">
      ...
      <div className="w-full max-w-[1180px] mx-auto px-[16px] sm:px-[30px]">
        ...
        <GalleryRow items={row3Items} rowLabel="Gallery Row 3" />
      </div>
    </section>
  );
}
